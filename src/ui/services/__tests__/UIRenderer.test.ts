...
describe('UIRenderer', () => {
  ...
  describe('renderContainer', () => {
    ...
    test('должен создать кнопку "Сохранить"', () => {
      ...
      const saveButton = container.querySelector('button[onclick*="saveAllBlocksUI"]');
    ...
    test('должен создать кнопку "Очистить все"', () => {
      ...
      const clearButton = container.querySelector('button[onclick*="clearAllBlocksUI"]');
    ...
    test('должен показать предупреждение если контейнер не найден', () => {
      ...
      invalidRenderer.renderContainer();
