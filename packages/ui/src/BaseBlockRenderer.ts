...
export class BaseBlockRenderer {
    ...
    getRender(block: BaseBlockModel, stringTemplate: string): HTMLElement | null {
        return this.createFallbackBlock(block, stringTemplate);
    }

    createFallbackBlock(block: BaseBlockModel, renderBoilerplate: string): HTMLElement | null {
        let renderBlock = this.decodeHTMLEntities(this.renderTemplate(renderBoilerplate, block))

        // найти в renderBoilerplate все вхождения по шаблону [[key]] и заменить на innerHTML из block.props[key]
        const parser = new DOMParser();
        const doc = parser.parseFromString(renderBlock, 'text/html');
        const resultHTMLElement = doc?.body?.firstChild

        return (resultHTMLElement as HTMLElement) || null;
    }
    ...
    private processTextNode(node: Text, context: any) {
        const newValue = node.nodeValue!.replace(
            /\[\[\s*([^\]]+?)\s*\]\]/g,
            (_, expr) => {
                const v = this.evaluateExpression(expr, context)
                return v
            }
        );
        ...
        try {
            ...
            if (typeof value === 'object') {
                if (value instanceof HTMLElement) {
                    const htmlString = value.outerHTML;
