class y {
  ...
  getRender(e, r) {
    return this.createFallbackBlock(e, r);
  }
  createFallbackBlock(e, r) {
    var n;
    let t = this.decodeHTMLEntities(this.renderTemplate(r, e));
  ...
  renderTemplate(e, r) {
    ...
    return this.processForDirectives(o.body, r.props), this.processConditionalRendering(o.body, r.props), this.processTemplateExpressions(o.body, r.props), o.body.innerHTML;
  }
  processConditionalRendering(e, r) {
    e.querySelectorAll("[render-if]").forEach((o) => {
      const s = o.getAttribute("render-if");
      o.removeAttribute("render-if"), this.evaluateCondition(r, s) || o.remove();
    });
  }
  processTemplateExpressions(e, r) {
    ...
    for (; t.nextNode(); ) {
      ...
      o.nodeType === Node.TEXT_NODE ? this.processTextNode(o, r) : o.nodeType === Node.ELEMENT_NODE && this.processElementAttributes(o, r);
    }
  }
  processTextNode(e, r) {
    const t = e.nodeValue.replace(
      /\[\[\s*([^\]]+?)\s*\]\]/g,
      (o, s) => this.evaluateExpression(s, r)
    );
  ...
  renderCollection(e, r, t, o, s) {
    const i = e.parentNode, n = document.createDocumentFragment(), p = Array.isArray(r);
    Object.entries(r).forEach(([l, a], d) => {
      ...
      this.processNode(m, f), n.appendChild(m);
    }), i.replaceChild(n, e);
  }
  processNode(e, r) {
    this.processElementAttributes(e, r), this.processConditionalRendering(e, r);
    const t = document.createTreeWalker(
      e,
    ...
    for (; t.nextNode(); ) {
      const o = t.currentNode;
      o.nodeType === Node.TEXT_NODE ? this.processTextNode(o, r) : o.nodeType === Node.ELEMENT_NODE && this.processNode(o, r);
    }
  }
  getNestedProperty(e, r) {
    return r.split(".").reduce((t, o) => {
      const s = o.match(/^(\w+)\[(\d+)\]$/);
      return s && Array.isArray(t == null ? void 0 : t[s[1]]) ? t[s[1]][Number(s[2])] : t == null ? void 0 : t[o];
    }, e);
  }
}
var b = /* @__PURE__ */ ((c) => (c.ADD = "add", c.EDIT = "edit", c.REMOVE = "remove", c.MOVE_NEXT = "move-next", c.MOVE_PREV = "move-prev", c))(b || {});
const T = `
...
`;
function A(c) {
  return typeof c == "string" || Array.isArray(c) ? !(c != null && c.length) : typeof c == "object" ? !Object.keys(c).some(Boolean) : !c;
}
class F extends y {
  constructor(e, r, t, o) {
    super(), this.blocksData = e, this.renderers = r, this.manager = t, this.renderHTMLContainer = o, this.renderOptMain(), this.renderOptServices();
  }
  get registeredBlocks() {
    return this.manager.getRegisteredBlocks();
  }
  renderOptMain() {
    this.renderHTMLContainer.innerHTML = "", this.blocksData.forEach((e, r) => {
      const t = this.renderers[e.type], o = this.createFallbackBlock(e, t), s = this.getRenderControls(e), i = this.getRenderControl(e, b.ADD);
      ...
      if (!n)
        return;
      this.renderHTMLContainer.appendChild(n), this.renderHTMLContainer.querySelectorAll("[data-bb-control]").forEach((l) => {
        l.addEventListener("click", (a) => {
          if (!(a != null && a.currentTarget) || !(a.currentTarget instanceof HTMLElement))
            return;
          const d = a.currentTarget.getAttribute("data-type");
          d && ([b.EDIT, b.ADD].includes(d) && this.renderModalFromControlType(d, a), d === b.REMOVE && (console.log("сквозное сообщение что блок удален"), this.removeBlockFromControl(a)), d === b.MOVE_PREV && this.moveBlockFromControl(b.MOVE_PREV, a), d === b.MOVE_NEXT && this.moveBlockFromControl(b.MOVE_NEXT, a));
        });
      });
    });
  }
  renderOptServices() {
    var o;
    const r = new DOMParser().parseFromString(C, "text/html"), t = (o = r == null ? void 0 : r.body) == null ? void 0 : o.firstChild;
    t && document.body.append(t);
  }
  getRenderControls(e) {
    const r = document.createElement("div");
    return r.classList.add("bb-block-controls", "bb-border-box"), [b.EDIT, b.REMOVE, b.MOVE_PREV, b.MOVE_NEXT].forEach((t) => {
      const o = this.getRenderControl(e, t);
      o && r.append(o);
    }), r;
  }
  getRenderControl(e, r) {
  ...
  moveBlockFromControl(e, r) {
    var s, i;
    const t = (i = (s = r == null ? void 0 : r.target) == null ? void 0 : s.closest("[data-bb-type-block]")) == null ? void 0 : i.getAttribute("id"), o = this.blocksData.findIndex((n) => n.id === t);
    !this.blocksData[o] || !t || (e === b.MOVE_PREV && this.manager.movePrevBlock(o), e === b.MOVE_NEXT && this.manager.moveNextBlock(o), this.updateDataFromManager(), setTimeout(() => {
      this.scrollToBlock(t);
    }, 100));
  }
  scrollToBlock(e) {
    var r;
    (r = document.getElementById(e)) == null || r.scrollIntoView({ behavior: "smooth" });
  }
  removeBlockFromControl(e) {
    var t, o;
    const r = (o = (t = e == null ? void 0 : e.target) == null ? void 0 : t.closest("[data-bb-type-block]")) == null ? void 0 : o.getAttribute("id");
    r && this.removeBlock(r);
  }
  removeBlock(e) {
  ...
  createFields(e) {
    let r = [];
    for (let t in e.form) {
      const o = e.form[t], s = e.props[t], i = A(s) ? null : s, n = this.createField(o, t, i);
      n && r.push(n);
    }
    return r;
  }
  createField(e, r, t) {
    ...
    if (e.typeField === "each-array") {
      ...
      const o = e.each.typeField === "each-object" ? this.createField(e.each, r, t) : this.createField(e.each, r);
      let s = [];
      Array.isArray(t) && t.length && e.each.typeField !== "each-object" && t.forEach((n) => {
        s.push(
          this.createField(e.each, r, n)
        );
      }), console.log("value.label", e.label);
      const i = e.each.typeField === "each-object" ? o : this.getRender(
        {
          id: `bb-fieldset-${e.typeField}-${r}`,
    ...
    if (e.typeField === "each-object") {
      ...
      let o = [];
      for (let p in e.each) {
        const l = this.createField(e.each[p], p);
        l && o.push(l);
      }
      ...
      Array.isArray(t) && t.forEach((p) => {
        let l = [];
        for (let d in p) {
          const m = p[d], f = this.createField(e.each[d], d, m);
          f && l.push(f);
        }
        const a = this.getRender(
          {
            id: `bb-fieldset-${e.typeField}-${r}`,
            type: `bb-fieldset-${e.type}`,
            props: {
  ...
  renderModalFromControlType(e, r) {
    var i, n;
    let t = null;
    if (e === b.ADD && (t = this.getRender(
      {
        id: "bb-window-add",
        type: "bb-window-add",
        props: {
      ...
      const p = (n = (i = r == null ? void 0 : r.target) == null ? void 0 : i.closest("[data-bb-type-block]")) == null ? void 0 : n.getAttribute("id"), l = this.blocksData.find((m) => m.id === p);
      let a = this.createFields(l);
      t = this.getRender(
        {
          id: "bb-form-edit",
          type: "bb-form-edit",
    ...
    s == null || s.forEach((p) => {
      p.addEventListener("click", (l) => {
        if (!(l.currentTarget instanceof HTMLElement))
          return;
        const a = l.currentTarget.getAttribute("data-item"), d = a ? this.registeredBlocks[a] : null;
        let m = this.createFields(d);
  ...
  createModal(e) {
    if (!e)
      return;
    const r = document.querySelector("[data-bb-modal]");
    r && r.remove(), e.addEventListener("click", (t) => {
