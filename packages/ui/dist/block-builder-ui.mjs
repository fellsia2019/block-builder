...
var L = (l, e, t) => e in l ? C(l, e, { enumerable: !0, configurable: !0, writable: !0, value: t }) : l[e] = t;
var h = (l, e, t) => (L(l, typeof e != "symbol" ? e + "" : e, t), t);
class M {
  constructor() {
  }
  getRender(e, t) {
    return this.createFallbackBlock(e, t);
  }
  createFallbackBlock(e, t) {
    var n;
    let r = this.decodeHTMLEntities(this.renderTemplate(t, e));
    const s = new DOMParser().parseFromString(r, "text/html");
    return ((n = s == null ? void 0 : s.body) == null ? void 0 : n.firstChild) || null;
  }
  ...
  renderTemplate(e, t) {
    ...
    return this.processForDirectives(o.body, t.props), this.processConditionalRendering(o.body, t.props), this.processTemplateExpressions(o.body, t.props), o.body.innerHTML;
  }
  processConditionalRendering(e, t) {
    e.querySelectorAll("[render-if]").forEach((o) => {
      const s = o.getAttribute("render-if");
      o.removeAttribute("render-if"), this.evaluateCondition(t, s) || o.remove();
    });
  }
  processTemplateExpressions(e, t) {
    ...
    for (; r.nextNode(); ) {
      ...
      o.nodeType === Node.TEXT_NODE ? this.processTextNode(o, t) : o.nodeType === Node.ELEMENT_NODE && this.processElementAttributes(o, t);
    }
  }
  processTextNode(e, t) {
    const r = e.nodeValue.replace(
      /\[\[\s*([^\]]+?)\s*\]\]/g,
      (o, s) => this.evaluateExpression(s, t)
    );
    r !== e.nodeValue && (e.nodeValue = r);
  }
    ...
    try {
      ...
      if (typeof o == "object") {
        if (o instanceof HTMLElement) {
          const s = o.outerHTML, i = document.createElement("div");
      ...
      return console.warn(`Error evaluating expression "${e}":`, r), "";
    }
  }
  processElementAttributes(e, t) {
    Array.from(e.attributes).forEach((r) => {
      r.value.includes("[[") && e.setAttribute(
        r.name,
        r.value.replace(
          /\[\[\s*([^\]]+?)\s*\]\]/g,
          (o, s) => this.getPropertyValue(t, s)
        )
      );
    });
  }
  getPropertyValue(e, t) {
    const r = t.split(".").reduce((o, s) => o == null ? void 0 : o[s], e);
    return r == null ? "" : typeof r == "object" ? JSON.stringify(r, null, 2) : String(r);
  }
  evaluateCondition(e, t) {
    if (t.startsWith("!"))
      return !this.getPropertyValue(e, t.substring(1));
    if (t.includes("===")) {
      const [o, s] = t.split("===").map((i) => i.trim());
      return this.getPropertyValue(e, o) === s;
    }
    const r = this.getPropertyValue(e, t);
    return ["number", "boolean"].includes(typeof r) ? Boolean(r) : !!r;
  }
  processForDirectives(e, t) {
    Array.from(e.querySelectorAll("[render-for]")).forEach((o) => {
      const s = o.getAttribute("render-for");
      o.removeAttribute("render-for");
      const { itemName: i, indexName: n, collectionPath: a } = this.parseForExpression(s);
      if (!i || !a) {
        console.warn(`Invalid render-for expression: ${s}`);
        return;
      }
      const d = this.getNestedProperty(t, a);
      if (!d) {
        o.remove();
        return;
      }
  ...
  parseForExpression(e) {
    const r = e.replace(/\s*,\s*/g, ",").trim().split(/\s+/).filter(Boolean);
    if (r.length === 3 && r[1] === "in") {
      const o = r[0].match(/^\((\w+),(\w+)\)$/);
      if (o)
        return {
          itemName: o[1],
          indexName: o[2],
  ...
  renderCollection(e, t, r, o, s) {
    const i = e.parentNode, n = document.createDocumentFragment(), a = Array.isArray(t);
    Object.entries(t).forEach(([d, b], c) => {
      const p = e.cloneNode(!0), m = {
        ...s,
        [r]: b,
        ...o ? { [o]: a ? c : d } : {},
        $key: `${d}-${c}`,
      ...
      this.processNode(p, m), n.appendChild(p);
    }), i.replaceChild(n, e);
  }
  processNode(e, t) {
    this.processElementAttributes(e, t), this.processConditionalRendering(e, t);
    const r = document.createTreeWalker(
      e,
    ...
    for (; r.nextNode(); ) {
      const o = r.currentNode;
      o.nodeType === Node.TEXT_NODE ? this.processTextNode(o, t) : o.nodeType === Node.ELEMENT_NODE && this.processNode(o, t);
    }
  }
  getNestedProperty(e, t) {
    return t.split(".").reduce((r, o) => {
      const s = o.match(/^(\w+)\[(\d+)\]$/);
      return s && Array.isArray(r == null ? void 0 : r[s[1]]) ? r[s[1]][Number(s[2])] : r == null ? void 0 : r[o];
    }, e);
  }
}
var f = /* @__PURE__ */ ((l) => (l.ADD = "add", l.EDIT = "edit", l.REMOVE = "remove", l.MOVE_NEXT = "move-next", l.MOVE_PREV = "move-prev", l))(f || {});
const F = `
...
class $ {
  ...
  parse(e) {
    const t = {};
    for (const [r, o] of Object.entries(this.schema))
      o.typeField === "each-array" ? t[r] = this._parseArrayField(e, r, o) : t[r] = this._parseSimpleField(e, r);
    return t;
  }
  _parseSimpleField(e, t) {
    const r = e.querySelector(`[data-bb-field-key="${t}"]`);
    if (!r)
      return "";
    const o = r.querySelector(".bb-base-field__field");
    return o ? o.value : "";
  }
  _parseArrayField(e, t, r) {
    const o = e.querySelector(`[data-bb-field-key="${t}"]`);
    if (!o)
      return [];
    const s = o.querySelectorAll(".bb-fieldset__item"), i = [];
    return s.forEach((n) => {
      if (r.each.typeField === "each-object") {
        const a = {};
        for (const [d, b] of Object.entries(r.each.each)) {
          const c = n.querySelector(`[data-bb-field-key="${d}"]`);
          if (c) {
            const p = c.querySelector(".bb-base-field__field");
            a[d] = p ? p.value : "";
          }
        }
        i.push(a);
      } else {
        const a = n.querySelector(".bb-base-field__field");
        a && a.value && i.push(a.value);
      }
    }), i;
  }
}
class D {
  constructor(e, t) {
    ...
    h(this, "parser");
    h(this, "formElement");
    this.model = new k(t), this.parser = new $(t), this.formElement = e;
  }
  getFormData() {
    const e = this.parser.parse(this.formElement);
    return this.model.update(e), this.model.getData();
  }
}
function H(l) {
  return typeof l == "string" || Array.isArray(l) ? !(l != null && l.length) : typeof l == "object" ? !Object.keys(l).some(Boolean) : !l;
}
class R extends M {
  constructor(e, t, r, o) {
    super(), this.blocksData = e, this.renderers = t, this.manager = r, this.renderHTMLContainer = o, this.renderOptMain(), this.renderOptServices();
  }
  get registeredBlocks() {
    return this.manager.getRegisteredBlocks();
  }
  renderOptMain() {
    this.renderHTMLContainer.innerHTML = "", this.blocksData.forEach((e, t) => {
      const r = this.renderers[e.type], o = this.createFallbackBlock(e, r), s = this.getRenderControls(e), i = this.getRenderControl(e, f.ADD);
      if (i == null || i.classList.add("bb-block-control-add"), !o)
        return;
      const n = this.getRender(
        {
          id: e.id,
          type: e.type,
          name: "",
          props: {
            controlAddTop: i && t === 0 ? i : null,
            controlAddBottom: i || null,
            controls: s,
            userBlock: o,
            id: e.id,
            type: e.type
      ...
      if (!n)
        return;
      this.renderHTMLContainer.append(n), n.querySelectorAll("[data-bb-control]").forEach((d) => {
        d.addEventListener("click", (b) => {
          if (!(b != null && b.currentTarget) || !(b.currentTarget instanceof HTMLElement))
            return;
          const c = b.currentTarget.getAttribute("data-type");
          if (c) {
            if ([f.EDIT, f.ADD].includes(c)) {
              this.renderModalFromControlType(c, b);
              const p = document.querySelector("[data-bb-modal]"), m = p == null ? void 0 : p.querySelector("[data-bb-form]");
              m == null || m.addEventListener("submit", (u) => {
                u.preventDefault();
                const T = new D(m, e.form).getFormData();
                console.log("!!!!!!!!!!!!!!!", T);
              });
            }
            c === f.REMOVE && (console.log("сквозное сообщение что блок удален"), this.removeBlockFromControl(b)), c === f.MOVE_PREV && this.moveBlockFromControl(f.MOVE_PREV, b), c === f.MOVE_NEXT && this.moveBlockFromControl(f.MOVE_NEXT, b);
          }
        });
      });
    });
  ...
  renderOptServices() {
    var o;
    const t = new DOMParser().parseFromString(w, "text/html"), r = (o = t == null ? void 0 : t.body) == null ? void 0 : o.firstChild;
    r && document.body.append(r);
  }
  getRenderControls(e) {
    const t = document.createElement("div");
    return t.classList.add("bb-block-controls", "bb-border-box"), [f.EDIT, f.REMOVE, f.MOVE_PREV, f.MOVE_NEXT].forEach((r) => {
      const o = this.getRenderControl(e, r);
      o && t.append(o);
    }), t;
  }
  getRenderControl(e, t) {
  ...
  moveBlockFromControl(e, t) {
    var s, i;
    const r = (i = (s = t == null ? void 0 : t.target) == null ? void 0 : s.closest("[data-bb-type-block]")) == null ? void 0 : i.getAttribute("id"), o = this.blocksData.findIndex((n) => n.id === r);
    !this.blocksData[o] || !r || (e === f.MOVE_PREV && this.manager.movePrevBlock(o), e === f.MOVE_NEXT && this.manager.moveNextBlock(o), this.updateDataFromManager(), setTimeout(() => {
      this.scrollToBlock(r);
    }, 100));
  }
  scrollToBlock(e) {
    var t;
    (t = document.getElementById(e)) == null || t.scrollIntoView({ behavior: "smooth" });
  }
  removeBlockFromControl(e) {
    var r, o;
    const t = (o = (r = e == null ? void 0 : e.target) == null ? void 0 : r.closest("[data-bb-type-block]")) == null ? void 0 : o.getAttribute("id");
    t && this.removeBlock(t);
  }
  removeBlock(e) {
    this.manager.removeBlock(e), this.updateDataFromManager();
  }
  updateDataFromManager() {
    this.blocksData = this.manager.getBlocks(), this.renderOptMain();
  }
  createFields(e) {
    let t = [];
    for (let r in e.form) {
      const o = e.form[r], s = e.props[r], i = H(s) ? null : s, n = this.createField(o, r, i);
      n && t.push(n);
    }
    return t;
  }
  createField(e, t, r) {
    if (e.typeField, e.typeField === "text") {
      const o = this.getRender(
        {
          id: `bb-field-${e.typeField}-${t}`,
          ...
          props: {
            ...
            // вынести в утилс, такой же генератор айди есть в BlockManager
            key: t
          }
        },
        x
    ...
    if (e.typeField === "each-array") {
      ...
      const o = e.each.typeField === "each-object" ? this.createField(e.each, t, r) : this.createField(e.each, t);
      let s = [];
      Array.isArray(r) && r.length && e.each.typeField !== "each-object" && r.forEach((n) => {
        s.push(
          this.createField(e.each, t, n)
        );
      });
      const i = e.each.typeField === "each-object" ? o : this.getRender(
        {
          id: `bb-fieldset-${e.typeField}-${t}`,
    ...
    if (e.typeField === "each-object") {
      ...
      let o = [];
      for (let a in e.each) {
        const d = this.createField(e.each[a], a);
        d && o.push(d);
      }
      const s = this.getRender(
        {
          id: `bb-fieldset-${e.typeField}-${t}`,
          type: `bb-fieldset-${e.type}`,
          props: {
      ...
      Array.isArray(r) && r.forEach((a) => {
        let d = [];
        for (let c in a) {
          const p = a[c], m = this.createField(e.each[c], c, p);
          m && d.push(m);
        }
        const b = this.getRender(
          {
            id: `bb-fieldset-${e.typeField}-${t}`,
            type: `bb-fieldset-${e.type}`,
            props: {
  ...
  renderModalFromControlType(e, t) {
    var i, n;
    let r = null;
    if (e === f.ADD && (r = this.getRender(
      {
        id: "bb-window-add",
        type: "bb-window-add",
        props: {
      ...
      const a = (n = (i = t == null ? void 0 : t.target) == null ? void 0 : i.closest("[data-bb-type-block]")) == null ? void 0 : n.getAttribute("id"), d = this.blocksData.find((p) => p.id === a);
      let b = this.createFields(d);
      r = this.getRender(
        {
          id: "bb-form-edit",
          type: "bb-form-edit",
    ...
    this.createModal(o);
    const s = o == null ? void 0 : o.querySelectorAll("[data-item]");
    s == null || s.forEach((a) => {
      a.addEventListener("click", (d) => {
        if (!(d.currentTarget instanceof HTMLElement))
          return;
        const b = d.currentTarget.getAttribute("data-item"), c = b ? this.registeredBlocks[b] : null;
        let p = this.createFields(c);
        const m = this.getRender(
          {
            id: `bb-form-${e}`,
            type: `bb-form-${e}`,
            props: {
  ...
  createModal(e) {
    if (!e)
      return;
    const t = document.querySelector("[data-bb-modal]");
    t && t.remove(), e.addEventListener("click", (r) => {
      !r.target || !(r.target instanceof HTMLElement) || (r.target.hasAttribute("data-bb-modal-close") || r.target.hasAttribute("data-bb-modal")) && (e.remove(), document.body.classList.remove("bb-scrollbar-is-locked"));
    }), document.body.classList.add("bb-scrollbar-is-locked"), document.body.append(e);
  }
}
export {
  M as BaseBlockRenderer,
  R as OptBlockRenderer
};
